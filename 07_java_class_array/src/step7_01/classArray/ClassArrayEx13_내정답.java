...
class Manager_1{
	...
	StudentEx_1 remove_StudentEx (int index) { // check 값 대입
		StudentEx_1 del_st = list[index]; // 링크 공유
		if(stdCnt == 1) {
			list = null;
		}
		else if(stdCnt > 1) {
			StudentEx_1 [] temp = list; // 탈퇴할 id 기준으로 앞쪽 정보 저장
			list = new StudentEx_1[stdCnt-1];
		...
		stdCnt--;
		return del_st; // list = null이 되었으나 del_st의 값은 살아있음.
	}
	...
	public static void main(String[] args) {
		...
		while (true) {
 		...
 		if		(sel == 1) { // 가입 check_id, add_StudentEx 메서드 사용
			...
			else if (sel == 2) { // 탈퇴: check_id, remove_StudentEx 메서드 사용
				...
				if(check == -1) {
					...
					StudentEx_1 del_st = manager.remove_StudentEx(check);
					System.out.println(del_st + "님 탈퇴 되었습니다.");
				}
			}
